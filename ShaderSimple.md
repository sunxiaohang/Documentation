...
#### 后处理效果
将renderer>>post processing layer组件添加到主相机，设置处理层级，抗锯齿，触发层的gameObject
#### 后处理效果
##### 环境光遮蔽AO
- 效果：模拟在彼此靠近的折痕，孔和表面中出现的柔和阴影，这些区域被遮挡环境光，因此会更暗
- 实时渲染性能消耗大 采用烘培的方式
##### 抗锯齿
- FXAA快速近似：不支持运动矢量的移动设备快速算法
- SMAA亚像素形态：不支持运动矢量的移动设备高质量算法
- TAA时间抗锯齿：需要运动矢量支持，高性能设备
##### 自动曝光
- 自动曝光效果模拟人眼调节以适应实时的亮度变化，他会动态调整图像的曝光度来匹配其中间色调
- 原理：在每帧生成一个直方图，对齐进行过滤找到平均亮度值{需要Compute shader支持}
##### Bloom
- bloom效果使得图像焕发明亮的区域，会产生从图像明亮区域延伸出来的条纹，模拟现实世界相机在光线淹没镜头时产生的效果
- 还可以模拟"污垢"，在整个屏幕上涂抹污渍或灰尘以衍射bloom效果
##### 色差
色差效果沿图像边界将颜色分为红色，绿色和蓝色通道，当光线折射并导致波长分散在镜头中时，重现了真实世界相机产生的效果
##### 颜色分级
- 地清晰度范围LDR：适用于低端平台，将分级应用于夹紧在[0,1]范围内并存储在标准LUT中的最终渲染帧
- 高清晰度范围HDR：适合支持HDR渲染的平台，所有色彩操作都在HDR中，存储在3D对数编码的LUT中，以确保足够的范围覆盖和精度
- 外部：与外部软件中编写的自定义3D LUT一起使用
- 色调映射可以将HDR重新映射到LDR中，使地动态范围图像看起来像具有较高的动态范围
##### 景深
当前景中的对象保持聚焦时，景深效果会模糊图像的背景，模拟真实世界相机镜头的聚焦特性
##### 颗粒
在颗粒效应加上胶片噪声在图像中，unity的库里效果基于相干渐变噪声。
##### 镜头变形
镜头失真效果模拟失真所引起的真实世界照相机透镜的形状
##### 运动模糊
运动模糊效果模拟方向上的图像相机运动，当真实世界的相机在镜头光圈打开的情况下移动或捕捉移动物体速度超过相机曝光时间时，模拟相机产生的模糊效果
##### 插图
会使图像的边缘变暗，将注意力吸引到图像中心